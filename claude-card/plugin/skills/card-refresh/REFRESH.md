...
0 9,11,13,15,17,19 * * 1-5  /Users/<you>/Documents/code/claude-desktop-buddy-repo/claude-card/plugin/skills/card-refresh/scripts/refresh_loop.sh
```
...
```bash
crontab -l                                    # 确认条目在
tail -f ~/.claude-card-refresh.log            # 看 cron 日志
```
...
```cron
0 */2 * * *  /usr/bin/python3 /path/to/claude-card/plugin/skills/card-refresh/scripts/fallback_refresh.py
```
...
短期：注释 crontab 那行。
长期：删掉 crontab 那行 + `rm ~/.claude-card-refresh.log`。

刷新不会自动重启，只有 cron 在跑就会一直来。

## 成本日志

每次 headless Claude 调用都会打 token 用量到 `~/.claude-card-refresh.log`
