...
description: |
  ...
  没反应" / "刷固件" / "怎么连卡片" / "ai-desk-card 连不上".
trigger_keywords:
  - card onboard
  - 入职
  - 第一次用
...
> ai-desk-card daemon 还没启动。我帮你拉起。
...
如果还不行，把 probe 输出和 `tail -20 /tmp/ai_desk_card_daemon.log` 给用户看。
...
- "为什么屏幕一直显示之前的内容" → e-ink 关机后 0 功耗保留最后一帧，这是
  特性不是 bug。
- "我想换显示的人名 / 二维码" → 编辑 `ai-desk-card/assets/profile.yaml`，
  跑 `/card-sleep` 推送名片帧。
...
```bash
bash $CLAUDE_PLUGIN_ROOT/skills/card-onboard/scripts/probe.sh
tail -30 /tmp/ai_desk_card_daemon.log
