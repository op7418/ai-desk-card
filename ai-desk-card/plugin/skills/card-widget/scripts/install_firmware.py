#!/usr/bin/env python3
"""ai-desk-card firmware OTA installer.

Modes:
  --detect   Print current firmware version on the connected card (parses
             [boot] m5-paper-buddy vXX or [boot] ai-desk-card vXX lines).
...
def detect_version(port: str | None = None) -> str:
    ...
    try:
        ...
        for line in txt.splitlines():
            if "[boot]" in line and ("ai-desk-card" in line or "m5-paper-buddy" in line):
                return line.strip()
...
def download(url: str, expected_sha: str) -> str:
    out = "/tmp/ai_desk_card_firmware.bin"
